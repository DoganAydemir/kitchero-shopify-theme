...
- **SEO & Structured Data:** Implement valid JSON-LD structured data for SEO. The Kitchero `snippets/structured-data.liquid` already emits Product, Article, Breadcrumb, Organization, and WebSite blocks. For any additions, follow Shopify's `structured_data` filter docs and Schema.org definitions directly — do NOT consult Dawn or Horizon. This is a mandatory Theme Store requirement.
...
- **Color settings:** use the Shopify-native `color_scheme` setting type (tied to a `color_scheme_group`) instead of raw `color` settings where possible. This integrates with Shopify's color scheme system and is what merchants expect.
- **CSS class names (Crucial Tailwind Rule):** Do NOT output Tailwind utility classes in the Liquid files. You must translate the Tailwind design from the Next.js source into plain, vanilla CSS using the BEM methodology (e.g., `.kt-hero`, `.kt-hero__title`). The final Shopify theme will not have a Tailwind build step. Create component-specific CSS files (e.g., `assets/kt-section-hero.css`) and load them only in the sections that need them via `{{ 'kt-section-hero.css' | asset_url | stylesheet_tag }}`. The established convention is `assets/kt-section-<slug>.css` for section CSS and `assets/kt-<name>.css` for snippet/global CSS — `kt-` prefix everywhere matches the BEM namespace.
...
- **JavaScript:** no framework, no build step (unless explicitly added later). Vanilla JS with custom elements where reactive behavior is needed. One file per interactive section: `assets/kt-section-hero.js`, `assets/kt-section-main-product.js`. Load them via `<script src="{{ 'kt-section-hero.js' | asset_url }}" defer="defer"></script>` from inside the corresponding `sections/*.liquid`. The `kt-` prefix matches the CSS naming convention.
